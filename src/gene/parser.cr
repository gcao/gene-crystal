...
module Gene
  class Parser < StringScanner
    ...
    def self.parse(input, options = {} of Symbol => Object)
      new(input, options).parse
    end

    def initialize(source, options = {} of Symbol => Object)
    ...
    private def convert_encoding(source)
      if source.responds_to?(:to_str)
    ...
    UNESCAPE_MAP = Hash(Char, String).new
    UNESCAPE_MAP['"'] = "\""
    UNESCAPE_MAP['\\'] = "\\"
    UNESCAPE_MAP['/'] = "/"
    UNESCAPE_MAP['b'] = "\b"
    ...
    # EMPTY_8BIT_STRING = ""
    # if ::String.responds_to?(:encode)
    #   EMPTY_8BIT_STRING.force_encoding Encoding::ASCII_8BIT
    # end

    def parse_value(options = {} of Symbol => Object)
    ...
    def parse_string
      ...
      string = self[1].gsub(%r((?:\\[\\bfnrt"'/]|(?:\\u(?:[A-Fa-f\d]{4}))+|\\[\x20-\xff]))) do |c|
      ...
      if string.responds_to?(:force_encoding)
    ...
    def parse_group
      ...
      attribute_for_group = {} of String => Object
      ...
      return Gene::NOOP if result.size == 0 # NOOP
    ...
    def handle_processing_instructions(gene)
      gene.properties.each do |key, value|
        if DOCUMENT_INSTRUCTIONS.includes?(key)
          send "#{key}=", value
        end
      end
      result = Gene::UNDEFINED
      gene.data.each do |item|
        if DOCUMENT_INSTRUCTIONS.includes?(item.to_s)
